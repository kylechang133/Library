...
def login(email, password):
    ...
    cur.execute("SELECT * FROM members WHERE lower(email) = ? AND passwd = ?", (email.lower(), password))
...
def signup(email, password, name, byear, faculty):
    ...
    cur.execute("INSERT INTO members (email, passwd, name, byear, faculty) VALUES (?, ?, ?, ?, ?)", (email, password, name, byear, faculty))
    conn.commit()
    conn.close()    
