...
def view_profile(user):
    ...
    cur.execute(
        "SELECT count(bid) FROM borrowings WHERE member = ? AND end_date IS NOT NULL GROUP BY member",
        (email,),
    )
    ...
    cur.execute(
        "SELECT count(bid) FROM borrowings WHERE member = ? AND end_date IS NULL GROUP BY member",
        (email,),
    )
    returned_books = cur.fetchone()
    ...
    if returned_books is None:
        ...
        cur.execute(
            "SELECT start_date, count(bid) FROM borrowings WHERE member = ? AND end_date IS NULL GROUP BY member, start_date",
            (email,),
        )
    ...
    cur.execute(
        "SELECT count(pid), sum(amount), sum(paid_amount) FROM penalties, borrowings WHERE penalties.bid = borrowings.bid AND borrowings.member = ? AND (paid_amount < amount OR paid_amount IS NULL) GROUP BY member",
        (email,),
    )
    ...
    if penalty_data is None:
        ...
        print(
            f"You currently have: {penalty_data[0]} unpaid penalties. Your unpaid amount is ${penalty_data[1] - penalty_data[2]}."
        )
...
def return_book(user):
    ...
    cur.execute(
        """SELECT b1.bid, b2.title, b1.start_date, b2.book_id
    ...
    if not borrowing_list:
        input(
            "You are not currently borrowing any books, press ENTER to return to main menu...\n"
        )
    else:
        ...
        while True:
            ...
            for x in borrowing_list:
                ...
                print(
                    f"Borrowing id: {str(x[0])} | Title: {x[1]} | Borrowing date: {x[2]} | Return deadline: {deadline}"
                )
            ...
            if return_choice.lower() == "e":
                ...
                cur.execute(
                    ...
                    (date_to_string(datetime.today()), int(return_choice)),
                )
                ...
                if is_overdue(borrowing_date):
                    charge = (
                        datetime.today() - (borrowing_date + timedelta(days=20))
                    ).days
                    ...
                    cur.execute(
                        ...
                        (penalty_index, return_choice, charge, 0),
                    )
                    print(
                        "Return successful - you will be charged "
                        + str(charge)
                        + "$ due to overdue penalty.\n\n"
                    )
                else:
                ...
                while True:
                    review_choice = input(
                        "Would you wish to give a review to this book? (Y/N)\n"
                    )
                    review_rating = 0
                    if review_choice.lower() == "n":
                        ...
                        while True:
                            ...
                            try:
                                if int(review_rating) < 1 or int(review_rating) > 5:
                                    print(
                                        "Please provide the rating in the given range."
                                    )
                                else:
                                    break
                            except:
                        ...
                        review_text = input(
                            "If possible, please justify your rating with a brief description.\n"
                        )
                        if not review_text:
...
def search_book(user):
    ...
    while (
        True
    ):  # Outer most loop for the feature (exiting this loop means finishing the search function)
        ...
        while (
            continue_searching and not books_borrowed
        ):  # While loop that allows users to indefinitely search for books unless they decide not to
            books, total_count = book_search_engine(keyword, page)
            total_pages = max(
                (total_count + 4) // 5, 1
            )  # Math that decides what "page" it is currently displaying
            ...
            for book in books:
                print(
                    f"Book ID: {book['book_id']} | Title: {book['title']} | Author: {book['author']} | Publish Year: {book['pyear']} | "
            ...
            borrowing_option = ask_yes_no(
                "Would you like to borrow a book from this list?"
            )
            if borrowing_option:
                while True:
                    book_id_input = input(
                        "Enter the ID of the book you want to borrow, or 'E' to exit: "
                    )
                    ...
                    try:
                        ...
                        print(
                            "Please enter a valid ID for the book you want to borrow."
                        )
                        continue
                    ...
                    if book_id not in book_ids_available:
                        print(
                            "The book you want to borrow does not exist in the books listed."
                        )
                        continue
...
def book_search_engine(keyword, page=1):
    ...
    offset = (
        page - 1
    ) * page_size  # Calculates how many items should be skipped before starting to return results for the current page
    ...
    """
    cur.execute(
        query, (keyword, keyword, keyword, keyword, keyword, keyword, page_size, offset)
    )
...
def borrow_book(user, book_id):
    ...
    cur.execute(
        "SELECT COUNT(*) FROM borrowings WHERE book_id = ? AND end_date IS NULL",
        (book_id,),
    )
    if cur.fetchone()[0] > 0:
...
def pay_penalty(user):
    ...
    cur.execute(
        "SELECT pid, amount, paid_amount FROM penalties WHERE bid IN (SELECT bid FROM borrowings WHERE member = ?) AND paid_amount < amount",
        (email,),
    )
    ...
    if not penalties:
        ...
        while x == 0:
            ...
            for penalty in penalties:
                print(
                    f"Penalty ID: {penalty[0]} | Amount: ${penalty[1]} | Paid Amount: ${penalty[2]}"
                )
            ...
            if penalty_select == "exit":
                ...
                try:
                    ...
                    penalty = next(
                        (p for p in penalties if p[0] == penalty_select), None
                    )
                    # Make sure penalty belongs to given user
                    cur.execute(
                        "SELECT COUNT(*) FROM penalties WHERE pid = ? AND bid IN (SELECT bid FROM borrowings WHERE member = ?)",
                        (penalty_select, email),
                    )
                    ...
                    if penalty is not None:
                        while x == 0:
                            ...
                            if payment_amount <= 0:
                                ...
                                cur.execute(
                                    "UPDATE penalties SET paid_amount = paid_amount + ? WHERE pid = ?",
                                    (payment_amount, penalty_select),
                                )
                                conn.commit()
